...
import { MopError } from '../utils/error'
...
function readFeedUpdateHeaders(headers: Headers): FeedUpdateHeaders {
  const feedIndex = headers.get('cluster-feed-index')
  const feedIndexNext = headers.get('cluster-feed-index-next')

  if (!feedIndex) {
    throw new MopError('Response did not contain expected cluster-feed-index!')
  }

  if (!feedIndexNext) {
    throw new MopError('Response did not contain expected cluster-feed-index-next!')
  }
