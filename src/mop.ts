import * as mop from './modules/mop'
...
import { MopArgumentError, MopError, MopResponseError } from './utils/error'
...
import { ReferenceType } from './cid'

/**
 * The main component that abstracts operations available on the main Mop API.
 *
 * Not all methods are always available as it depends in what mode is Mop node launched in.
 * For example gateway mode and light node mode has only limited set of endpoints enabled.
 */
export class Mop {
  /**
   * URL on which is the main API of Mop node exposed
   */
  ...
  /**
   * @param url URL on which is the main API of Mop node exposed
   * @param options
   */
   ...
   * @see [Mop docs - Upload and download](https://docs.bnbcluster.org/docs/access-the-cluster/upload-and-download)
   * @see [Mop API reference - `POST /bytes`](https://docs.bnbcluster.org/api/#tag/Bytes/paths/~1bytes/post)
   */
  async uploadData(
    ...
    return bytes.upload(this.getKy(options), data, voucherBatchId, options)
  }

  /**
   * Download data as a byte array
   *
   * @param reference Mop data reference in hex string (either 64 or 128 chars long) or ENS domain.
   * @param options Options that affects the request behavior
   * @throws TypeError if some of the input parameters is not expected type
   * @throws MopArgumentError if there is passed ENS domain with invalid unicode characters
   * @see [Mop docs - Upload and download](https://docs.bnbcluster.org/docs/access-the-cluster/upload-and-download)
   * @see [Mop API reference - `GET /bytes`](https://docs.bnbcluster.org/api/#tag/Bytes/paths/~1bytes~1{reference}/get)
   */
   ...
   *
   * @param voucherBatchId Voucher BatchId to be used to upload the chunk with
   * @param data    Raw chunk to be uploaded
   * @param options Additional options like tag, encryption, pinning, content-type and request options
   *
   * @returns reference is a content hash of the data
   * @see [Mop docs - Upload and download](https://docs.bnbcluster.org/docs/access-the-cluster/upload-and-download)
   * @see [Mop API reference - `POST /chunks`](https://docs.bnbcluster.org/api/#tag/Chunk/paths/~1chunks/post)
   */
  async uploadChunk(voucherBatchId: string | BatchId, data: Uint8Array, options?: UploadOptions): Promise<Reference> {
    assertBatchId(voucherBatchId)
    ...
    if (data.length < SPAN_SIZE) {
      throw new MopArgumentError(`Chunk has to have size of at least ${SPAN_SIZE}.`, data)
    }

    if (data.length > CHUNK_SIZE + SPAN_SIZE) {
      throw new MopArgumentError(`Chunk has to have size of at most ${CHUNK_SIZE}.`, data)
    }

    if (options) assertUploadOptions(options)

    return chunk.upload(this.getKy(options), data, voucherBatchId, options)
  }

  /**
   * Download chunk as a byte array
   *
   * @param reference Mop chunk reference in hex string (either 64 or 128 chars long) or ENS domain.
   * @param options Options that affects the request behavior
   * @throws TypeError if some of the input parameters is not expected type
   * @throws MopArgumentError if there is passed ENS domain with invalid unicode characters
   * @see [Mop docs - Upload and download](https://docs.bnbcluster.org/docs/access-the-cluster/upload-and-download)
   * @see [Mop API reference - `GET /chunks`](https://docs.bnbcluster.org/api/#tag/Chunk/paths/~1chunks~1{reference}/get)
   */
  ...
  /**
   * Upload single file to a Mop node.
   *
   ...
   * @see [Mop docs - Upload and download](https://docs.bnbcluster.org/docs/access-the-cluster/upload-and-download)
   * @see [Mop API reference - `POST /mop`](https://docs.bnbcluster.org/api/#tag/File/paths/~1mop/post)
  ...
  async uploadFile(
    ...
    if (isFile(data)) {
      ...
      return addCidConversionFunction(
        await mop.uploadFile(this.getKy(options), fileData, voucherBatchId, fileName, fileOptions),
        ReferenceType.MANIFEST,
      )
    } else if (isReadable(data) && options?.tag && !options.size) {
      const result = await mop.uploadFile(this.getKy(options), data, voucherBatchId, name, options)
      await this.updateTag(options.tag, result.reference)

      return addCidConversionFunction(result, ReferenceType.MANIFEST)
    } else {
      return addCidConversionFunction(
        await mop.uploadFile(this.getKy(options), data, voucherBatchId, name, options),
        ReferenceType.MANIFEST,
      )
    }
  }
   ...
   *
   * @param reference Mop file reference in hex string (either 64 or 128 chars long), ENS domain or Cluster CID.
   ...
   * @see Data
   * @see [Mop docs - Upload and download](https://docs.bnbcluster.org/docs/access-the-cluster/upload-and-download)
   * @see [Mop API reference - `GET /mop`](https://docs.bnbcluster.org/api/#tag/Collection/paths/~1mop~1{reference}~1{path}/get)
   */
  async downloadFile(
    ...
    return mop.downloadFile(this.getKy(options), reference, path)
  }
  ...
  async downloadReadableFile(
    ...
    return mop.downloadFileReadable(this.getKy(options), reference, path)
  }

  /**
   * Upload collection of files to a Mop node
   *
   ...
   *
   * @see [Mop docs - Keep your data alive / Voucher stamps](https://docs.bnbcluster.org/docs/access-the-cluster/keep-your-data-alive)
   * @see [Mop docs - Upload directory](https://docs.bnbcluster.org/docs/access-the-cluster/upload-a-directory/)
  ...
  async uploadFiles(
    ...
    return addCidConversionFunction(
      await mop.uploadCollection(this.getKy(options), data, voucherBatchId, options),
      ReferenceType.MANIFEST,
    )
  }
  ...
  async uploadCollection(
    ...
    return addCidConversionFunction(
      await mop.uploadCollection(this.ky, collection, voucherBatchId, options),
      ReferenceType.MANIFEST,
    )
  }
   ...
   * @see [Mop API reference - `POST /tags`](https://docs.bnbcluster.org/api/#tag/Tag/paths/~1tags/post)
   */
   ...
   * @throws MopArgumentError if limit or offset have invalid options
   *
   * @see [Mop docs - Syncing / Tags](https://docs.bnbcluster.org/docs/access-the-cluster/syncing)
   * @see [Mop API reference - `GET /tags`](https://docs.bnbcluster.org/api/#tag/Tag/paths/~1tags/get)
   */
  ...
  /**
   * Retrieve tag information from Mop node
   *
   * **Warning! Not allowed when node is in Gateway mode!**
   *
   ...
   *
   * @see [Mop docs - Syncing / Tags](https://docs.bnbcluster.org/docs/access-the-cluster/syncing)
   * @see [Mop API reference - `GET /tags/{uid}`](https://docs.bnbcluster.org/api/#tag/Tag/paths/~1tags~1{uid}/get)
   *
   */
   ...
   * @param options Options that affects the request behavior
   * @throws TypeError if tagUid is in not correct format
   * @throws MopResponse error if something went wrong on the Mop node side while deleting the tag.
   *
   * @see [Mop docs - Syncing / Tags](https://docs.bnbcluster.org/docs/access-the-cluster/syncing)
   * @see [Mop API reference - `DELETE /tags/{uid}`](https://docs.bnbcluster.org/api/#tag/Tag/paths/~1tags~1{uid}/delete)
   */
   ...
   *
   * @see [Mop docs - Syncing / Tags](https://docs.bnbcluster.org/docs/access-the-cluster/syncing)
   * @see [Mop API reference - `PATCH /tags/{uid}`](https://docs.bnbcluster.org/api/#tag/Tag/paths/~1tags~1{uid}/patch)
   */
  ...
  /**
   * Instructs the Mop node to reupload a locally pinned data into the network.
   *
   * @param reference Mop data reference to be re-uploaded in hex string (either 64 or 128 chars long) or ENS domain.
   * @param options Options that affects the request behavior
   * @throws MopArgumentError if the reference is not locally pinned
   * @throws TypeError if some of the input parameters is not expected type
   * @throws MopArgumentError if there is passed ENS domain with invalid unicode characters
   *
   * @see [Mop API reference - `PUT /stewardship`](https://docs.bnbcluster.org/api/#tag/Stewardship/paths/~1stewardship~1{reference}/put)
   */
   ...
   *
   * @param reference Mop data reference to be checked in hex string (either 64 or 128 chars long) or ENS domain.
   * @param options Options that affects the request behavior
   * @throws TypeError if some of the input parameters is not expected type
   * @throws MopArgumentError if there is passed ENS domain with invalid unicode characters
   *
   * @see [Mop API reference - `GET /stewardship`](https://docs.bnbcluster.org/api/#tag/Stewardship/paths/~1stewardship~1{reference}/get)
   */
  ...
  async isFeedRetrievable(
    ...
    if (type !== 'sequence') {
      throw new MopError('Only Sequence type of Feeds is supported at the moment')
    }
  ...
  /**
   * Send data to recipient or target with Postal Service for Cluster.
   *
   ...
   * **Warning! Not allowed when node is in Gateway mode!**
   *
   * **Warning! If the recipient Mop node is a light node, then he will never receive the message!**
   * This is because light nodes does not fully participate in the data exchange in Cluster network and hence the message won't arrive to them.
   *
   * @param voucherBatchId Voucher BatchId that will be assigned to sent message
   * @param topic Topic name
   * @param target Target message address prefix. Has a limit on length. Recommend to use `Utils.Pss.makeMaxTarget()` to get the most specific target that Mop node will accept.
   ...
   * @see [Mop API reference - `POST /pss`](https://docs.bnbcluster.org/api/#tag/Postal-Service-for-Cluster/paths/~1pss~1send~1{topic}~1{targets}/post)
   */
  async pssSend(
    voucherBatchId: string | BatchId,
    topic: string,
    ...
    if (recipient) {
      ...
      return pss.send(this.getKy(options), topic, target, data, voucherBatchId, recipient)
    } else {
      return pss.send(this.getKy(options), topic, target, data, voucherBatchId)
    }
  }

  /**
   * Subscribe to messages for given topic with Postal Service for Cluster
   *
   * **Warning! Not allowed when node is in Gateway mode!**
   *
   * **Warning! If connected Mop node is a light node, then he will never receive any message!**
   * This is because light nodes does not fully participate in the data exchange in Cluster network and hence the message won't arrive to them.
   *
   * @param topic Topic name
   ...
   *
   * @see [Mop docs - PSS](https://docs.bnbcluster.org/docs/dapps-on-cluster/pss)
   * @see [Mop API reference - `GET /pss`](https://docs.bnbcluster.org/api/#tag/Postal-Service-for-Cluster/paths/~1pss~1subscribe~1{topic}/get)
   */
  pssSubscribe(topic: string, handler: PssMessageHandler): PssSubscription {
    ...
    ws.onerror = ev => {
      ...
      if (!cancelled) {
        handler.onError(new MopError(ev.message), subscription)
      }
    }
  ...
  /**
   * Receive message with Postal Service for Cluster
   *
   * Because sending a PSS message is slow and CPU intensive,
   * it is not supposed to be used for general messaging but
   * most likely for setting up an encrypted communication
  ...
  async pssReceive(topic: string, timeoutMsec = 0): Promise<Data> {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (timeoutMsec > 0) {
        ...
        timeout = setTimeout(() => {
          subscription.cancel()
          reject(new MopError('pssReceive timeout'))
   ...
   *
   * Feed manifest chunk allows for a feed to be able to be resolved through `/mop` endpoint.
   *
   * @param voucherBatchId  Voucher BatchId to be used to create the Feed Manifest
   * @param type            The type of the feed, can be 'epoch' or 'sequence'
   * @param topic           Topic in hex or bytes
   * @param owner           Owner's BNB Smart Chain address in hex or bytes
   * @param options Options that affects the request behavior
   *
   * @see [Mop docs - Feeds](https://docs.bnbcluster.org/docs/dapps-on-cluster/feeds)
   * @see [Mop API reference - `POST /feeds`](https://docs.bnbcluster.org/api/#tag/Feed/paths/~1feeds~1{owner}~1{topic}/post)
   */
  async createFeedManifest(
    ...
    const reference = await createFeedManifest(this.getKy(options), canonicalOwner, canonicalTopic, voucherBatchId, {
      type,
    })
   ...
   *
   * The default Signer of Mop instance is used if `options.signer` is not specified.
   ...
   *
   * @throws MopError if `options.signer` is not specified nor the default Signer on Mop's instance is specified.
   *
   * @see [Mop docs - Feeds](https://docs.bnbcluster.org/docs/dapps-on-cluster/feeds)
   */
  async setJsonFeed<T extends AnyJson>(
    ...
    return setJsonData(this, writer, voucherBatchId, data, options)
  }
   ...
   * @param options.address BNB Smart Chain address of owner of the feed that signed it. This option is exclusive with `signer` option.
   * @param options.type Type of Feed
   *
   * @see [Mop docs - Feeds](https://docs.bnbcluster.org/docs/dapps-on-cluster/feeds)
   */
  async getJsonFeed<T extends AnyJson>(topic: string, options?: JsonFeedOptions): Promise<T> {
    ...
    if (options?.signer && options?.address) {
      throw new MopError('Both options "signer" and "address" can not be specified at one time!')
    }
    ...
    if (options?.address) {
      ...
      try {
        ...
        if (e instanceof MopError) {
          throw new MopError('Either address, signer or default signer has to be specified!')
        } else {
          throw e
        }
      }
   ...
   *
   * @param ownerAddress The BNB Smart Chain address of the owner
   * @param options Options that affects the request behavior
   * @see [Mop docs - Chunk Types](https://docs.bnbcluster.org/docs/dapps-on-cluster/chunk-types#single-owner-chunks)
   */
  ...
  /**
   * Ping the Mop node to see if there is a live Mop node on the given URL.
   *
   * @param options Options that affects the request behavior
   * @throws If connection was not successful throw error
   */
  async checkConnection(options?: RequestOptions): Promise<void> | never {
    assertRequestOptions(options, 'VoucherBatchOptions')
   ...
   * @param signer
   * @private
   * @throws MopError if either no Signer was passed or no default Signer was specified for the instance
   */
  private resolveSigner(signer?: Signer | Uint8Array | string): Signer {
    ...
    throw new MopError('You have to pass Signer as property to either the method call or constructor! Non found.')
  }
