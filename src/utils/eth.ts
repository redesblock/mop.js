...
/**
 * Check if this is checksummed BNB Smart Chain address
 *
 * @param address Ethereum address as hex string
 */
...
/**
 * Check if is valid BNB Smart Chain address
 *
...
function assertClusterNetworkId(networkId: number): asserts networkId is number {
  if (Number.isInteger(networkId && networkId > 0 && networkId < Number.MAX_SAFE_INTEGER)) {
    throw new TypeError('cluster network id must be positive integer')
  }
}

/**
 * Get cluster overlay address from public BNB Smart Chain address and cluster network id
 *
 * @param ethAddress  Public BNB Smart Chain address
 * @param networkId   Cluster network id
 *
 * @return Cluster overlay address
 */
export function ethToClusterAddress(ethAddress: string | HexString | HexEthAddress, networkId = 1): OverlayAddress {
  assertEthAddress(ethAddress)
  assertClusterNetworkId(networkId)
