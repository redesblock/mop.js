...
import { MopArgumentError, MopError } from './error'
import { isFile } from './file'
import { assertHexString, assertPrefixedHexString, isHexString } from './hex'
import { isReadable } from './stream'
import { decodeCid, encodeReference, ReferenceType } from '../cid'
...
export function assertNonNegativeInteger(value: unknown, name = 'Value'): asserts value is number | NumberString {
  ...
  if (Number(value) < 0) throw new MopArgumentError(`${name} has to be bigger or equal to zero`, value)
}

export function assertPositiveInteger(value: unknown, name = 'Value'): asserts value is number | NumberString {
  assertInteger(value, name)

  if (Number(value) <= 0) throw new MopArgumentError(`${name} has to be bigger then zero`, value)
}
...
export function assertReferenceOrEns(value: unknown): asserts value is ReferenceOrEns {
   ...
   * adg-.ets - INVALID
   * as-a.com - VALID
   * bnbcluster.org - VALID
   * http://asdf.asf - INVALID
   * řš+ýí.šě+ř.čě - VALID
   * tsg.asg?asg - INVALID
   * tsg.asg:1599 - INVALID
   * bnbcluster.something- - INVALID
   * bnbcluster.-something - INVALID
   * bnbcluster.some-thing - VALID
   *
  ...
  // adds 160kB minified size which is significant. We expects that full validation will be done on Mop side.
...
/**
 * Function that mainly converts Cluster CID into hex encoded Cluster Reference
 *
 * @param value
...
export function makeReferenceOrEns(value: unknown, expectedCidType: ReferenceType): ReferenceOrEns {
  ...
  try {
    ...
    if (result.type !== expectedCidType) {
      throw new MopError(
        `CID was expected to be of type ${expectedCidType}, but got instead ${result.type ?? 'non-Cluster CID'}`,
      )
    }

    return result.reference
  } catch (e) {
    if (e instanceof MopError) throw e
  }
 ...
 * @param result
 * @param cidType Type as described in the @ethersphere/cluster-cids-js -> ReferenceType
 */
...
export function assertVoucherBatchOptions(value: unknown): asserts value is VoucherBatchOptions {
  if (value === undefined) {
    return
  }
  ...
  const options = value as VoucherBatchOptions
...
export function assertAllTagsOptions(entry: unknown): asserts entry is AllTagsOptions {
  ...
  if (options?.limit !== undefined) {
    ...
    if (options.limit < TAGS_LIMIT_MIN) {
      throw new MopArgumentError(`AllTagsOptions.limit has to be at least ${TAGS_LIMIT_MIN}`, options.limit)
    }

    if (options.limit > TAGS_LIMIT_MAX) {
      throw new MopArgumentError(`AllTagsOptions.limit has to be at most ${TAGS_LIMIT_MAX}`, options.limit)
    }
  }
