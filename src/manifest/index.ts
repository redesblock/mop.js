import { Mop, Utils } from '..'
import { loadAllNodes, MantarayNode, MetadataMapping, Reference } from '../mantaray'
...
export class ManifestJs {
  ...
  public async isManifest(hash: string): Promise<boolean> {
    try {
      const data = await this.mop.downloadData(hash)
  ...
  public async resolveFeedManifest(hash: string): Promise<string | null> {
    ...
    const reader = this.mop.makeFeedReader('sequence', topic, owner)
  ...
  private async load(reference: Uint8Array) {
    return this.mop.downloadData(Utils.bytesToHex(reference))
  }
