import { MopArgumentError } from '../utils/error'
...
export function makeSpan(length: number): Bytes<8> {
  if (length <= 0) {
    throw new MopArgumentError('invalid length for span', length)
  }

  if (length > MAX_SPAN_LENGTH) {
    throw new MopArgumentError('invalid length (> MAX_SPAN_LENGTH)', length)
  }
