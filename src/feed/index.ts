...
export async function findNextIndex(
  ...
  try {
    ...
    if (e instanceof MopResponseError && e.status === 404) {
...
export async function updateFeed(
  ...
  return uploadSingleOwnerChunkData(ky, signer, voucherBatchId, identifier, payloadBytes, options)
}
...
export function makeFeedWriter(ky: Ky, type: FeedType, topic: Topic, signer: Signer): FeedWriter {
  const upload = async (
    ...
    return updateFeed(ky, signer, topic, canonicalReference, voucherBatchId, { ...options, type })
  }
